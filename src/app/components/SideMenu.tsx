...
const lists=list.map((li)=>
<li>
            <details className="group [&_summary::-webkit-details-marker]:hidden">
              <summary className="  bg-blue-500 flex cursor-pointer items-center justify-between rounded-lg px-4 py-2 text-white hover:bg-blue-300 hover:text-gray-700">
                <span className="text-sm font-medium "> {li.name} </span>

              {li.subMenu.length>0 && <span className="shrink-0 transition duration-300 group-open:-rotate-180">
                  <svg
              ...
              {li.subMenu.length>0  && <ul className="mt-2 space-y-1 px-4">
                 {li.subMenu.map((sub,index)=>(
                    <li key={index}>
                    <a
                      href={sub.url}
  ...
  return (
    ...
    <div
      style={{ display: props.closedMenu ? "block" : "none" }}
      className={` 
        
      transition transform ease-in-out 	duration-1000   flex h-screen w-96 flex-col justify-between border-e bg-white z-50
    ...
    >
      <div className="px-4 py-6 ">
        <div className="flex justify-between">
          ...
          <CloseIcon
            className="cursor-pointer hover:bg-gray-200 rounded-full  text-black"
            onClick={() => {
              props.setClosedMenu(!props.closedMenu);
            }}
          />
        </div>
